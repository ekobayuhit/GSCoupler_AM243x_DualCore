  ...
  if (offset && RPDOIndex >= 0x1400)
    {
      ...
      if (offset <= lastIndex)
        {
          /* get the CobId */
          pwCobId = (short unsigned int*)d->objdict[offset].pSubindex[1].pObject;
        ...
        while (offsetObjdict <= lastIndex)
          {
            switch (status)
              ...
              case state2:
                pwCobId = (short unsigned int*)d->objdict[offsetObjdict].pSubindex[1].pObject;
...
{
  // printf("Sendpdo cobId=0x%3x pdonum = %d\n", UNS16_LE(pdo->cob_id), pdoNum);
  // DebugP_log("Sendpdo cobId=0x%3x pdonum = %d\r\n", UNS16_LE(pdo->cob_id), pdoNum);
...
sendOnePDOevent (CO_Data * d, UNS8 pdoNum)
{
  ...
  if (buildPDO (d, pdoNum, &pdo))
    {
      MSG_ERR (0x3907, " Couldn't build TPDO number : ",
               pdoNum);
      // DebugP_log(" Couldn't build TPDO number : %d \r\n", pdoNum);
      return 0;
    }
  ...
  if (d->PDO_status[pdoNum].last_message.cob_id == pdo.cob_id
    ...
    )
    {
      ...
      // DebugP_log(" No changes -> go to next pdo : %d \r\n", pdoNum);
      return 0;
    }
  else
    {
            ...
            case state3:
              if ( /* bSubCount always 5 with objdictedit -> check disabled */
                   ...
                   pObject & 0x80000000)
                {
                  ...
                  MSG_WAR (0x3961, "COBID =",*(UNS32 *) d->objdict[offsetObjdict].pSubindex[1].pObject);
                  MSG_WAR (0x3962, "offsetObjdict=",offsetObjdict);
                  /*Go next TPDO */
                  status = state11;
                  break;
                }
              ...
              pTransmissionType =
                (UNS8 *) d->objdict[offsetObjdict].pSubindex[2].pObject;
              MSG_WAR (0x3963, "Reading PDO at index : ", 0x1800 + pdoNum);
              MSG_WAR (0x3964, "isSyncEvent : ", isSyncEvent);
              ...
              if (isSyncEvent &&
                   ...
                   *pTransmissionType))
                {
                  ...
                  MSG_WAR (0x3965, "  PDO is on SYNCHRO. Trans type : ",
                           *pTransmissionType);
                  memset(&pdo, 0, sizeof(pdo));
                  /*{
