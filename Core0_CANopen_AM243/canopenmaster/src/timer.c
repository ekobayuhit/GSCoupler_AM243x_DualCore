...
static uint64_t TimeCNT = 0;
static uint64_t NextTime = 0;
static uint64_t last_time_set = 0;

void CANTimerCallBack(void)
{
    TimeCNT = ClockP_getTimeUsec();

    /* Check using signed difference to handle overflow safely */
    if ((int32_t)(TimeCNT - NextTime) >= 0)
    {
...
{
    TimeCNT = ClockP_getTimeUsec();
    NextTime = TimeCNT + value;   // overflow-safe by design
}

TIMEVAL getElapsedTime(void)
{
    TimeCNT = ClockP_getTimeUsec();
    return (uint32_t)(TimeCNT - last_time_set); // overflow-safe
}
...
TIMER_HANDLE SetAlarm(CO_Data* d, UNS32 id, TimerCallback_t callback, TIMEVAL value, TIMEVAL period)
{
	...
	s_timer_entry *row;
	// eprintf("SetAlarm: id=%lu, value=%llu, period=%llu\n", id, value, period);
	...
	for(row_number=0, row=timers; row_number <= last_timer_raw + 1 && row_number < MAX_NB_TIMER; row_number++, row++)
	{
		...
		{	/* just store */
			...
			if (total_sleep_time > elapsed_time && total_sleep_time - elapsed_time > real_timer_value)
			{
				...
				// eprintf("SetAlarm: setting timer to %llu\n", real_timer_value);
