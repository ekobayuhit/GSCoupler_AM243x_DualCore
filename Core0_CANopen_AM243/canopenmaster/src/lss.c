...
UNS8 sendMaster_LSSMessage(CO_Data* d, UNS8 command,void *dat1,void *dat2);
	...
	if(d->lss_transfer.command==LSS_IDENT_FASTSCAN){
		if(d->lss_transfer.FastScan_SM==LSS_FS_RESET){
   			...
   			if(d->lss_transfer.LSSanswer!=0){
   				...
   				//eprintf("BitChecked=%d, IDNumber=%x MASK=%x\n",d->lss_transfer.BitChecked,d->lss_transfer.IDNumber,Mask);
   				StartLSS_FS_TIMER();
   				sendMaster_LSSMessage(d,LSS_IDENT_FASTSCAN,0,0);
   				return;
   			}
   			else{ 
...
UNS8 sendMaster_LSSMessage(CO_Data* d, UNS8 command,void *dat1,void *dat2)
{
  ...
  case LSS_CONF_ACT_BIT_TIMING: /* Activate Bit Timing Parameters */
	...
	if(d->lss_transfer.baudRate!="none"){
		...
		MSG_ERR(0x1D1B, "Master_-> Baud rate not specified",0);
		d->lss_transfer.dat1=1;
    	/* If there is a callback, it is responsible of the error */
    	if(d->lss_transfer.Callback){
	    	(*d->lss_transfer.Callback)(d,d->lss_transfer.command);
...
#endif
  default:
   	MSG_ERR(0x1D1C, "send Master_ LSS command not implemented", command);
  	return 0xFF;
  }
...
UNS8 sendLSS(CO_Data* d, UNS8 command,void *dat1,void *dat2)
{
  ...
  else {/* It is a Master_ */
  	res = sendMaster_LSSMessage(d, command,dat1,dat2);
  }
  return res ;
}
...
** @return                                                                                         
**/ 
UNS8 proceedLSS_Master_(CO_Data* d, Message* m )
{ 
	UNS8 msg_cs;
	UNS32 Dat1=0;
	UNS8 Dat2=0;
	...
	{
		//MSG_WAR(0x3D0D, "Master_LSS proceedLSS; unexpected message arrived;command ", m->data[0]);
		//return 0;
		goto ErrorProcessMaster_;
	}
	
 	MSG_WAR(0x3D1E, "Master_LSS proceedLSS; command ", m->data[0]);
	
   	switch(msg_cs=m->data[0]){
   		case LSS_INQ_NODE_ID: /* Inquire Node-ID */
   			if(d->lss_transfer.command!=LSS_INQ_NODE_ID)goto ErrorProcessMaster_;
   			Dat1=m->data[1];
   			break;
   		case LSS_CONF_NODE_ID: /* Configure Node-ID */
   		case LSS_CONF_BIT_TIMING: /* Configure Bit Timing Parameters */
   		case LSS_CONF_STORE: /* Store Configured Parameters */
   			if(d->lss_transfer.command!=msg_cs)goto ErrorProcessMaster_;
   			Dat1=m->data[1];
   			Dat2=m->data[2];
   			break;
   		case LSS_INQ_VENDOR_ID: /* Inquire Identity Vendor-ID */
 		...
 		case LSS_SM_SELECTIVE_RESP: /* Switch Mode Selective response */
 			...
 			break;
   		case LSS_IDENT_NON_CONF_SLAVE: /* LSS identify non-configured remote slave */
   			if(d->lss_transfer.command!=LSS_IDENT_REMOTE_NON_CONF)goto ErrorProcessMaster_;
   			break;
   		default:
   			MSG_ERR(0x1D1F, "Master_ LSS command not implemented", msg_cs);
  			return 0xFF;
   	}
...
ErrorProcessMaster_:
    MSG_WAR(0x3D20, "Master_LSS proceedLSS; unexpected message arrived;command ", m->data[0]);
	return 0xFF;
...
{
	return sendMaster_LSSMessage(d,command,dat1,dat2);
}*/
...
//void _lss_StoreConfiguration(UNS8 *error, UNS8 *spec_error){eprintf("_lss_StoreConfiguration\n");}
